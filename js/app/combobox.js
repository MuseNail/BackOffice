...
export function combobox({ groups = [], value = '', text = '', placeholder = '— pick —', onAdd = null, onAddText = null, addLabel = 'Add…', minWidth = 190, freeText = false, emptyText = 'No matches' } = {}) {
  ...
  function blockBgScroll(e) {
    ...
    // past the input — so closing is cleaner than trying to follow.) A touch INSIDE the panel
    // is left alone: the list scrolls itself, and we must never preventDefault it — on a short,
  ...
  function pick(v) {
    ...
    // closePanel()'s freeText branch reconciles whatever text is in the box — and after
