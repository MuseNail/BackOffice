...
const CHANGELOG = [
  { v: '0.71.10', items: [
    { icon: 'verified_user', t: 'The server itself now refuses a change headed into the wrong company’s books', d: 'Every change you make is now sealed with the name of the company whose books were open when you made it. Before saving anything, the server compares that seal to the books the change actually arrived at — and if they don’t match, it refuses to save it. A refused change isn’t lost: it’s held under Settings → Data & maintenance with an alert to you, already pointing at the company it was made in, one tap to file. Until now this protection lived only in the app; with the server checking too, even a future app bug that mis-addresses a change can’t quietly put it in another company’s ledger. Day to day nothing looks different — this only acts if something is genuinely wrong.' },
