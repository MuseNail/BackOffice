...
const CHANGELOG = [
  { v: '0.71.11', items: [
    ...
    { icon: 'call_split', t: 'Split a transaction across several vendors, with a note each', d: 'When you split a transaction across accounts, each split line can now carry its own vendor and its own note (tap “＋ detail” on a line to add them) — and you can reopen an already-split transaction to change them. Your Vendors report and totals now credit each split to the right vendor instead of lumping the whole transaction under one, and the QuickBooks export carries each split’s vendor and note through. A line you don’t tag still falls back to the transaction’s vendor, so nothing you already have changes.' },
  ] },
  { v: '0.71.10', items: [
