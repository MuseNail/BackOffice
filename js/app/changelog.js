...
const CHANGELOG = [
  { v: '0.71.21', items: [
    { icon: 'database', t: 'The offline copy of your books moved to roomier storage', d: 'Each device keeps a local copy of your books so the app opens instantly and works offline. That copy used to live in a small storage area (about 10 MB) shared by all your apps on this site — and for a large business it nearly filled it, which is what caused the earlier “can’t sign in” trouble. It now lives in IndexedDB, a much larger, disk-based store, so it can’t crowd out sign-in or your other apps again. The move happens automatically and safely: your data is copied to the new store before the old copy is removed, nothing that hasn’t synced yet is ever touched, and your sign-in is unaffected. One small trade-off you might notice: opening or switching a business can flash empty for a fraction of a second while the larger store loads. If a device is ever still stuck, Settings → Diagnostics → “Clear local cache” now frees both stores.' },
  ] },
  { v: '0.71.20', items: [
    { icon: 'person_check', t: 'A client’s vendor now always wins over your auto-categorize rules', d: 'When a client fills in a suggestion and names the vendor, that vendor should be what you see and approve. It already worked when the client picked an existing vendor from the list — but when they typed a vendor name, a memorized rule for that transaction could quietly replace it in Review, so approving recorded the rule’s vendor instead of the client’s. Now a client’s vendor — whether they pick one or type one — always overrides your rules and AI guesses in the Vendor field; approving keeps the client’s vendor (an existing name is matched, a new one is created). Your rules still fill in the vendor when the client didn’t suggest one, and your own manual pick still wins over everything. This was only the Vendor field on a single row — the account, invoice, note, and “Approve all” already used the client’s suggestion.' },
  ] },
  { v: '0.71.19', items: [
    { icon: 'swap_horiz', t: 'Clients can now flag a payment as a transfer to a bank or card', d: 'When a client fills in a waiting transaction in the client app, the account picker now has a “Transfer to a bank or card” group — so they can tell you “that checking withdrawal was a payment to the Visa” or “a move to savings,” not just pick an income/expense account. You still review and approve it, and it posts as a transfer (never as income or expense), with the matching row on the other account cleared automatically. A client can’t pick the account the transaction already belongs to (that would be a transfer to itself), and when they choose a transfer the Invoice field hides (transfers don’t have an invoice). This applies when a client assigns the whole transaction to one account; if they split it across accounts, any bank/card line still comes to you through “Review split” so transfers can’t hide inside a split and get double-counted.' },
    { icon: 'lock_open', t: 'A full device can no longer lock you out of sign-in', d: 'On a device where the browser’s storage was completely full (all three apps share one storage box on this site), signing in could fail with “can’t reach the server” even though the server was fine — the app just couldn’t save your sign-in. Now, when storage is full, the app automatically clears the disposable local copies of your data (which reload from the server) to make room, so sign-in and saving keep working — and it never clears anything that hasn’t synced yet. If a device is ever still wedged, Settings → Diagnostics has a new “Clear local cache” button, and a genuine storage problem now shows an honest message instead of a misleading network error.' },
  ] },
  { v: '0.71.18', items: [
    { icon: 'fact_check', t: 'Profit & Loss can now include items still in Review', d: 'The Profit & Loss report can now fold in transactions still waiting in Review, so your Total Income and Total Expense reflect work in progress instead of only what you’ve finished posting. There’s a new “Include items in Review” toggle above the P&L (on by default); when it’s on, each section shows an “Uncategorized income · in Review” / “Uncategorized expense · in Review” line and the totals include them. Important, so you can trust the numbers: this is a provisional estimate — items are counted simply by money in vs money out, so it can include transfers between your own accounts, credit-card payments, or refunds that aren’t really income or expense, and it may double-count money that’s already recorded elsewhere. Because of that, it deliberately does NOT change your Balance Sheet or your tax set-aside estimate (both stay on posted numbers only), so the P&L net won’t match them while items are in Review — that’s expected. Turn the toggle off any time for the strict posted-only statement. This is owner-only; the client view always shows posted entries only. Nothing about your books changed — this only reads what’s already in Review.' },
  ] },
  { v: '0.71.17', items: [
  ...
  { v: '0.71.15', items: [
    { icon: 'rule', t: 'Auto-categorize rules now match whole words — no more “Arco” catching “Marco”', d: 'Your rules used to match any run of letters, anywhere — so a rule for “Arco” (the gas station) also grabbed every “Marco …” payment, and a rule for “AAA” was quietly grabbing unrelated bank deposits because a random reference number inside them happened to contain the letters a-a-a. Rules now match whole words, so “Arco” matches “ARCO” and “ARCO 914” (and even “ARCO914”) but never “Marco”. Numbers and symbols still count as separators, so store numbers jammed onto a name still match. If a merchant name comes through jammed together (like “ubereats”), just use that whole word as the keyword. And when two of your rules could both match one transaction, the more specific one now wins (so “Bijan - American Home Design” beats a plain “American Home Design” rule) instead of whichever happened to be first. To match part of a word on purpose, use the “matches pattern” option and just type the letters to find anywhere. Nothing was changed in your books — this only affects the suggestions in Review going forward.' },
    { icon: 'search', t: 'Smarter header search', d: 'The search box at the top now puts exact and whole-word matches first, so searching “arco” shows Arco before Marco. You can still type part of a word to find things — those partial matches just sort lower.' },
  ] },
  { v: '0.71.14', items: [
    { icon: 'fact_check', t: 'Invoices now show whether each one is in your books — not just “paid” by Invoice2go', d: 'Until now an invoice’s “Paid” amount, its payments list, and the green “succeeded” tags all came straight from Invoice2go — none of it told you whether that money is actually recorded in your books. Now every invoice shows a plain status for exactly that. On the invoice screen (and as a small tag in the “In your books” column of the invoice list) you’ll see one of: green “✓ In your books” — the income is recorded in your books and linked to this invoice (it also shows how much); blue “Partly linked” — only part of what was paid is recorded so far; gray “Before your books” — the invoice was paid before your Back Office books began, so its income lives in your earlier records (for example, QuickBooks), not here; amber “Not linked” — reported paid, but nothing in your books is tied to it yet (its money may already be sitting as an uncategorized deposit or waiting in Review — use Reconcile income / Reconcile to bank to confirm and link it). This is read-only: it only reads what’s already there and changes no totals and no income. These labels are for you only — they don’t print on a customer’s invoice.' },
    { icon: 'bug_report', t: 'Fixed: a Review error that could break the screen and stop transactions from saving', d: 'When a client suggested a transaction through the client app and typed a brand-new vendor name (one you don’t already have on file), opening that row in Review could hit an error that quietly broke the whole Review screen — so approving those transactions didn’t stick. Nothing was lost: those transactions stayed waiting in Review the whole time. This is fixed — those rows now open and approve normally.' },
  ] },
  { v: '0.71.13', items: [
    { icon: 'receipt_long', t: 'Fixed: fully-paid invoices with an older date now import', d: 'The Invoice2go import skips invoices dated before your import cutoff (so it doesn’t re-post income QuickBooks already has). But when you convert an estimate to an invoice, Invoice2go keeps the estimate’s original — older — date, so a brand-new invoice paid recently could get that old date and be silently skipped (for example a September estimate paid in full in March). Now any invoice that is fully paid is imported even if its date is before your cutoff. This only adds the invoice record (it shows as paid) — it posts no income and changes nothing about your income, bank, or QuickBooks totals. The import preview now also shows how many invoices it skipped for being older than your cutoff and not fully paid, so nothing is dropped silently. (Invoices that are only partly paid and dated before the cutoff are still held back, so old balances don’t reappear as receivables.)' },
  ] },
  { v: '0.71.12', items: [
