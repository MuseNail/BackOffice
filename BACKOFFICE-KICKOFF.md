...
> **Status: BUILT THROUGH M13 — v0.17.0** (v0.12.0 LIVE at `musenail.github.io/BackOffice/`; v0.13.0–v0.17.0 need a Pages push + `wrangler deploy` + secrets: `SYNC_TOKEN`, `HELCIM_API_TOKEN`, `ANTHROPIC_API_KEY`). The Progress log at the bottom of this file is the build record. All planned milestones M0–M13 are built. v0.16.0–v0.17.0 are UX/hardening releases — no new milestones.
...
| M12 QuickBooks Desktop IIF export | 0.14.0 | ✅ built & locally verified | `lib/qb-iif.js` (full `!ACCNT` COA w/ qbName/qbType + fallback, `Parent:Child` subaccounts, balanced `!TRNS/!SPL` GENERAL JOURNAL blocks, CRLF, sanitized fields; 4 tests) + Settings export card (date range → `.iif` download; exported txns stamped `qbExportedAt`, overlapping re-export warns about doubling QB). DO `entity.bulkUpsert` now enforces txn invariants like single upserts. Verified in-browser: file content correct, stamps persist, duplicate warning fires. Final proof (owner): import a file into a real QB Desktop |
| M13 processor sync (Helcim) | 0.15.0 | ✅ built & locally verified | `lib/processor-match.js` (per-day gross from the LEDGER's clearing debits or Helcim's card-transactions; 1–3-day window matcher w/ fee-plausibility cap; 4 tests) + Review `⚡$` Match-deposit button: **books-first** — when the Muse mapping has a clearing account, the deposit posts bank +net / clearing −gross / fee +diff (income was already booked by the synced sales rows; clearing reconciles to $0.00 — verified to the penny), Helcim API fallback (`GET /b/:biz/processor/helcim/transactions`, `HELCIM_API_TOKEN` secret, 501 until set; mirrors Muse's field-verified route). **Square intentionally skipped — it's being retired in Muse; its deposits keep the manual `%` split.** Owner: set `HELCIM_API_TOKEN` + verify one real payout match |

| — UX hardening batch | 0.16.0 | ✅ built | Inline "＋ Add category" in Review (modal, no re-render data-loss); per-row memo field; Edit/Delete/Void in Ledger with reconciled-row protection; server-side delete guard for reconciled txns; archive modal with "preserved" note; Muse sync explanation paragraph; cash-position sub-text with account count |
| — Audit hardening batch | 0.17.0 | ✅ built | **CRITICAL:** snapshot() paginated loop (was truncating at 128 entries), bulkUpsert reconciliation guard (was a side door around the ledger), RECON-001 status lock server-side (voiding reconciled txns now rejected). **HIGH:** dead-code /_sync/inbound guard fixed; inventory.js `byId` shadow (broken restock posting); AI route uses tool use not `output_config`; vendor rule delete confirmation modal; device approve/revoke error feedback; skipped-row restore toggle in Review; period lock check before void; dead-letter log for 409-rejected writes. **MEDIUM/LOW:** reconciliation double-close guard, alreadyCents date-bound, 200-txn cap notice, broken-rule chip, login button disable-during-request, FileReader onerror, IIF archived-account filter, dashboard empty-state CTA, `sessCache` partial-eviction, service name stripped from /health, server-generated `createdAt`, listUsers pagination limit, WS size guard. See the full audit finding list (36 findings) in the session transcript. |

| — Plaid sandbox lockout + `/_` namespace close | 0.71.5 | ✅ built, Worker-only | **Incident (2026-06→07-16):** a Plaid feed was connected to *Honey - 8002* while `PLAID_ENV` was `sandbox`; Plaid's demo fixtures (Uber SF\*\*POOL\*\*, SparkFun, United Airlines) landed in the live Muse books — 18 staged, 5 approved into the ledger (−$32.39 into Boba/Misc Meals). Disconnecting the feed left the rows (`plaid-connect.js:75`). Fake data removed 2026-07-16 (Honey restored to $33,630.07; real Chase feed's 107 staged/59 posted untouched; backup `backoffice-backups/targets-plaid-sandbox-seq32019.json`). **Fixes:** `PLAID_HOSTS` drops `sandbox` entirely — the live Worker can only reach production, and any unrecognized `PLAID_ENV` now 501s (`plaid_env_invalid`) instead of silently falling back to fake data; `plaidEnv`/`plaidHost`/`configured` exported + 8 regression tests (`tests/plaid-env.test.mjs`). **SECURITY:** `/b/:biz/*` forwarded any path to the DO, which strips the prefix — so `GET /b/<biz>/_plaid/items` served Plaid **access_tokens in plaintext to any member** (verified live). The `/_` namespace is now default-denied (also closes `_sync/inbound` SYNC_TOKEN bypass + `_suggest` userId spoofing); `/_audit` stays for the Settings Activity card, scoped to owner/manager. **Open follow-ups (need a client release + version trio):** `plaid-connect.js:58/:111/:40` bucket every 501 as "add the Plaid secret" — should read `error === 'plaid_env_invalid'`; `plaid-connect.js:74` comment still describes the removed sandbox→real swap. **Separate backlog item:** `plaid.js` sync swallows Plaid errors → `{synced:0}` → client toasts "No new transactions", so a revoked feed reports success forever while the books fall behind. |

Decisions added during the build: transfers post bank↔bank with auto-matching of the opposite staged row (±7 days) so statements never double-count; deposit fee-splits post gross income + fee expense + net cash in one txn; subaccounts are one level deep via `parentId` with `Parent › Child` labels everywhere; AI suggestions are gated server-side by per-business budget/pause and always lose to rules/history; app-owner devices are approvable only by owner sessions. v0.17.0 adds reconciliation permanence (reconciled txns cannot be voided, status-locked server-side in both single-upsert and bulkUpsert paths).
